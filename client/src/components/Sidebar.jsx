...
const Sidebar = () => {
    ...
    const filteredUsers = input ? 
        safeUsers.filter((user) => user?.fullName?.toLowerCase().includes(input.toLowerCase())) : 
        safeUsers;
        ...
        <div className='flex flex-col'>
            {filteredUsers && filteredUsers.length > 0 ? (
                filteredUsers.map((user,index)=>(
                    <div key={user._id || index} onClick={() => setSelectedUser(user)} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser ?._id === user._id && 'bg-[#282142]/50'}`}>
